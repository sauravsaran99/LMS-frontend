...
import { getBranchMonthlyReport } from "../../api/report.api";
import PageMeta from "../../components/common/PageMeta";
import DatePicker from "../../components/form/date-picker";

const BranchMonthlyReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      setLoading(true);
      const res = await getBranchMonthlyReport(fromDate, toDate);
      setRows(res.data);
    } catch (err) {
  ...
  return (
    <>
      <PageMeta title="Branch Monthly Report" description="View monthly report by branch" />
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            View test bookings, completions, and revenue metrics by branch
          </p>
        </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-200 dark:border-gray-800">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                Report Results ({rows.length} {rows.length === 1 ? 'branch' : 'branches'})
              </h2>
            </div>
        ...
        {rows.length === 0 && !loading && (
          <div className="rounded-2xl border border-gray-200 border-dashed bg-gray-50 dark:border-gray-700 dark:bg-gray-800/20 p-12 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              Select a date range and apply the filter to view branch monthly reports.
            </p>
          </div>
        )}
      </div>
...
export default BranchMonthlyReport;
