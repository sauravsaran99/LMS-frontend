...
import { getTestMonthlyReport } from "../../api/report.api";
import PageMeta from "../../components/common/PageMeta";
import DatePicker from "../../components/form/date-picker";

const TestMonthlyReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      setLoading(true);
      const res = await getTestMonthlyReport(fromDate, toDate);
  ...
  return (
    <>
      <PageMeta title="Test Monthly Report" description="View monthly report by test" />
      <div className="space-y-6">
        {/* Page Header */}
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
            Test-wise Monthly Report
          </h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            View test bookings, completions, and revenue metrics by test
          </p>
        </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-200 dark:border-gray-800">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                Report Results ({rows.length} {rows.length === 1 ? 'test' : 'tests'})
              </h2>
            </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                        <div className="flex items-center gap-3">
                          <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-sm font-semibold">
                            {row.test_name?.charAt(0).toUpperCase()}
                          </div>
                          {row.test_name}
                        </div>
                      </td>
        ...
        {rows.length === 0 && !loading && (
          <div className="rounded-2xl border border-gray-200 border-dashed bg-gray-50 dark:border-gray-700 dark:bg-gray-800/20 p-12 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              Select a date range and apply the filter to view test monthly reports.
            </p>
          </div>
        )}
      </div>
...
export default TestMonthlyReport;
