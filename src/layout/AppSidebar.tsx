...
const AppSidebar: React.FC = () => {
  ...
  // const allowedNavItems = navItems.filter((item) => user && item.roles.includes(user?.role)); // This causes type error since user.role is string
  const allowedNavItems = navItems.filter((item) => {
    if (!user?.role) return false;
    // Cast to any to bypass strict type check for now, or ensure user.role is typed correctly in AuthContext
